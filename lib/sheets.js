...
const LOG_HEADERS = ['일시', '고유번호', '품종명', '전화번호', '채널', '상태'];
...
async function updateRow(rowNumber, updates) {
  ...
  for (const key of Object.keys(updates)) {
    ...
    data.push({ range: SHEET_NAME + '!' + columnLetter(colIndex + 1) + rowNumber, values: [[updates[key]]] });
  }
...
async function appendRow(obj) {
  return appendToSheet(SHEET_NAME, obj);
}
...
async function appendLog(log) {
  ...
  await appendToSheet(LOGS_SHEET, log);
}

async function getLogs() {
  await ensureSheet(LOGS_SHEET, LOG_HEADERS);
  const rows = await getRows(LOGS_SHEET);
  return rows.reverse();
}
...
module.exports = { getAllRows, getRows, findById, updateRow, appendRow, appendLog, getLogs, deleteLogRow };
